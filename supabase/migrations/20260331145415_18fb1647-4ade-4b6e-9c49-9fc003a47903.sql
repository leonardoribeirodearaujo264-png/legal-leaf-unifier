...
-- 2) Recreate sync_financeiro_to_parceiro_pagamento with recursion guard
...
-- 3) Recreate sync_parceiro_pagamento_to_financeiro with recursion guard
  ...
  IF pg_trigger_depth() < 2 THEN
    ...
    ELSE
      INSERT INTO fin_lancamentos (
        tipo, categoria_id, valor, descricao, data_vencimento, data_pagamento, origem, status, observacoes, created_by
      ) VALUES (
    ...
    END IF;
  ELSE
    -- In recursion: still need to handle INSERT for lancamento_financeiro_id assignment
    IF TG_OP = 'INSERT' AND NEW.lancamento_financeiro_id IS NULL THEN
      INSERT INTO fin_lancamentos (
        tipo, categoria_id, valor, descricao, data_vencimento, data_pagamento, origem, status, observacoes, created_by
      ) VALUES (
        CASE WHEN NEW.tipo = 'receber' THEN 'receita' ELSE 'despesa' END,
